...
from targets import Isotropic_assemblage,Isotropic_assemblage_2m,Isotropic_assemblage_3m

class Config:
    """ Config is just a collection of all metadata which can be accessed from auxiliary files as well. """
    def __init__(self):
        self.dim = 2 # Dimensionality of Hilbert-space, 2-5.
        self.oa = self.dim # Number of outcomes.
        self.ma = 2 # Number of measurements, 2 or 3.
        self.settings = np.array([[self.oa , self.ma]]) # Settings of untrusted party: [[oa,ma],[ob,mb],...].
        self.Det_strategy = deterministic_strategy(self.settings) # Det_strategy[i] represent the strategy of (i+1)th party.
        self.visibility = (0, .1, .2, .3, .4, .5, .6, .7, 1/sqrt(2), .71, .8, .9, 1) # Visibility parameter of the target assemblage.

        # Neural network parameters
        self.inputsize= self.oa**self.ma # oa^{ma}, i.e., the cardinality of the hidden variable.
        self.lambda_outputsize = 1 + 2*self.dim # variables for \sigma_\lambda , and one for weight distribution.
        self.latin_depth = 1
        ...
        self.weight_init_scaling = 2. # Default is 1. Set to larger values to get more variance in initial weights
        self.optimizer = 'adadelta' # Optimizer: 'adadelta', 'sgd', 'adam'
        self.learning_rate =  0.5
        self.decay = 0.001
        self.momentum = 0.25
        self.loss = 'td'    # 'fn'  'td'  'qre'  'hs'
    ...
    def change_astarget(self, v): # Change here the target assemblage imported from targets.py
        ...
        self.astarget = Isotropic_assemblage_2m(self.dim,v)
